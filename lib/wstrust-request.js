...
WSTrustRequest.prototype._populateRSTUsernamePassword = function(RSTTemplate, username, password) {
  var RST = RSTTemplate.replace(USERNAME_PLACEHOLDER, util.newSubstrSafe(username))
  RST = RST.replace(PASSWORD_PLACEHOLDER, util.newSubstrSafe(this._populatedEscapedPassword(password)));
  return RST;
};
