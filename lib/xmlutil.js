...
function expandQNames(xpath) {
  ...
  for (var i=0; i < pathParts.length; i++) {
    if (pathParts[i].indexOf(':') !== -1) {
      ...
      var expandedPath = XPATH_PATH_TEMPLATE.replace('LOCAL_NAME', util.newSubstrSafe(QNameParts[1]));
      expandedPath = expandedPath.replace('NAMESPACE', util.newSubstrSafe(namespaces[QNameParts[0]]));
